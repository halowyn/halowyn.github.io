...
## 样式
使用flexible进行适配的时候，字体大小要比设计稿的字体最少大两个字号
## 调查问卷
* app里面写了视口，影响到flexible.js的适配展示，想办法在引入flexible使用之前将视口移除，在flexiblejs之前写入script：

```
$('meta[name="viewport"]').remove();
```

* 引用js和css时，为了防止缓存，可以在其后加上版本号

```
<script  type=""text/javascript"" src="js/survey.js?v=Math.random()"></script>
```
